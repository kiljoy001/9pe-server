...
async fn prop_create_operations_logged_to_consensus() {
    ...
    runner.run(&strategy, |(filename, perm)| {
        futures::executor::block_on(async {
            ...
            match result.unwrap() {
                NinePeeMessage::Create { .. } => {
                    prop_assert!(blocks_added >= 1, "Create operation should log to consensus DAG");
                }
...
async fn prop_write_operations_logged_to_consensus() {
    ...
    runner.run(&strategy, |(data, offset)| {
        futures::executor::block_on(async {
            // Setup
            let temp_dir = TempDir::new().unwrap();
            let dag = Arc::new(BoundedGhostdag::new("test_consensus".to_string()));
            ...
            let _attach_result = handler.handle_attach(1, 0, "test".to_string(), "/".to_string()).await;
            let _walk_result = handler.handle_walk(1, 2, vec!["testfile".to_string()]).await;
            ...
            match result.unwrap() {
                NinePeeMessage::Write { .. } => {
                    let blocks_added = final_stats.total_blocks - initial_block_count;
                    prop_assert!(blocks_added >= 1, "Write operation should log to consensus DAG");
        ...
        futures::executor::block_on(async {
            ...
            match result.unwrap() {
                NinePeeMessage::Remove { .. } => {
                    let blocks_added = final_stats.total_blocks - initial_block_count;
                    prop_assert!(blocks_added >= 1, "Remove operation should log to consensus DAG");
                }
                NinePeeMessage::Error { .. } => {
...
async fn prop_consensus_blocks_maintain_integrity() {
    ...
    runner.run(&strategy, |data| {
        futures::executor::block_on(async {
            ...
            // Perform operations
            let _ = handler.handle_attach(1, 0, "test".to_string(), "/".to_string()).await;
            let _ = handler.handle_create(1, "testfile".to_string(), 0o644, 0).await;
            ...
            prop_assert!(final_stats.total_blocks <= 1000,

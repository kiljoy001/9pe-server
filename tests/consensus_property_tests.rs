...
use ninep_server::consensus::{
    BoundedGhostdag, NamespaceOp, Block, BlockState, BlockId,
...
fn namespace_op_strategy() -> impl Strategy<Value = NamespaceOp> {
    prop_oneof![
        ...
        (any::<String>(), any::<u32>(), any::<bool>()).prop_map(|(path, mode, is_dir)| {
            NamespaceOp::Create {
                path: format!("/{}", path.chars().filter(|c| c.is_alphanumeric()).collect::<String>()),
                mode: mode & 0o777,
                is_dir,
            }
        }),
        ...
        (any::<String>(), any::<String>()).prop_map(|(from, to)| {
            NamespaceOp::Rename {
                from: format!("/{}", from.chars().filter(|c| c.is_alphanumeric()).collect::<String>()),
                to: format!("/{}", to.chars().filter(|c| c.is_alphanumeric()).collect::<String>()),
            }
        }),
    ]
}
...
fn block_strategy(parent_ids: Vec<BlockId>) -> impl Strategy<Value = Block> {
    (
        ...
        any::<u64>(),
        any::<String>(),
    ).prop_map(move |(id, operations, timestamp, creator)| {
        Block {
            id: format!("block_{}", id.chars().filter(|c| c.is_alphanumeric()).take(10).collect::<String>()),
            parents: parent_ids.clone(),
            operations,
            timestamp,
            creator: creator.chars().filter(|c| c.is_alphanumeric()).take(10).collect::<String>(),
...
fn scaling_params_strategy() -> impl Strategy<Value = ScalingParams> {
    ...
    ).prop_map(|(min, max, initial, factor, up, down, window, cooldown)| {
        ScalingParams {
            min_size: min,
            max_size: max,
...
mod dynamic_scaling_tests {
    ...
    fn prop_scaling_params_valid() {
        ...
        runner.run(&scaling_params_strategy(), |params| {
    ...
    async fn prop_scaler_respects_bounds() {
        let strategy = (scaling_params_strategy(), prop::collection::vec((0.0f64..100.0, 0.0f64..1.0, 0u64..100), 10..100));
        let mut runner = proptest::test_runner::TestRunner::default();

        runner.run(&strategy, |(params, metrics)| {
            futures::executor::block_on(async {
                let scaler = Arc::new(DynamicScaler::new(params.clone()));

                // Record metrics
                for (throughput, fill_rate, fork_depth) in metrics {
                    scaler.record_metrics(throughput, fill_rate, fork_depth).await;
                }
    ...
    async fn prop_high_load_scales_up() {
        ...
        runner.run(&scaling_params_strategy(), |params| {
            futures::executor::block_on(async {
                ...
                // Wait past cooldown
                tokio::time::sleep(tokio::time::Duration::from_secs(params.cooldown_secs + 1)).await;
    ...
    async fn prop_low_load_scales_down() {
        ...
        runner.run(&scaling_params_strategy(), |params| {
            futures::executor::block_on(async {
                ...
                // Wait past cooldown
                tokio::time::sleep(tokio::time::Duration::from_secs(modified_params.cooldown_secs + 1)).await;
    ...
    async fn prop_scale_factor_applied() {
        ...
        runner.run(&scaling_params_strategy(), |params| {
            futures::executor::block_on(async {
                ...
                let expected_up = ((initial_size as f64 * params.scale_factor) as usize).min(params.max_size);
                prop_assert_eq!(new_up, expected_up);

                // Force scale down
                let new_down = scaler.apply_scale(ScaleDecision::ScaleDown, &params).await;
                let expected_down = ((new_up as f64 / params.scale_factor) as usize).max(params.min_size);
    ...
    async fn prop_prediction_reasonable() {
        ...
        runner.run(&strategy, |(params, horizon)| {
            futures::executor::block_on(async {
                let scaler = Arc::new(DynamicScaler::new(params.clone()));
...
mod integration_tests {
    ...
    fn prop_dag_with_dynamic_scaling() {
        let strategy = (prop::collection::vec(block_strategy(vec![]), 50..200), scaling_params_strategy());
        let mut runner = proptest::test_runner::TestRunner::default();

        runner.run(&strategy, |(blocks, params)| {
            futures::executor::block_on(async {
                ...
                for (i, mut block) in blocks.into_iter().enumerate() {
                    ...
                    block.parents = if i == 0 { vec![] } else { vec![last_id.clone()] };
        ...
        runner.run(&strategy, |ops| {
            futures::executor::block_on(async {
                ...
                for (i, op) in ops.into_iter().enumerate() {
                    let block = Block {
                        id: format!("block_{}", i),
                        parents: if i == 0 { vec![] } else { vec![format!("block_{}", i - 1)] },
                        operations: vec![op],
                        timestamp: i as u64,

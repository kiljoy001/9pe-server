...
fn test_e2e_namespace_manager_active() {
    let server = ExtensionsServer::start(17500, 17600, 17700).expect("Failed to start server");
    ...
    assert!(
        conn.is_ok(),
        "Server with namespace manager should be running"
    );
}
...
fn test_e2e_namespace_control_dir() {
    let server = ExtensionsServer::start(17501, 17601, 17701).expect("Failed to start server");
...
fn test_e2e_compute_namespace_available() {
    let server = ExtensionsServer::start(17502, 17602, 17702).expect("Failed to start server");
...
fn test_e2e_settrans_namespace_available() {
    let server = ExtensionsServer::start(17503, 17603, 17703).expect("Failed to start server");
...
fn test_e2e_mesh_networking_active() {
    let server = ExtensionsServer::start(17504, 17604, 17704).expect("Failed to start server");
...
fn test_e2e_client_list_virtual_dirs() {
    let server = ExtensionsServer::start(17505, 17605, 17705).expect("Failed to start server");
...
fn test_e2e_multiple_extensions_coexist() {
    let server = ExtensionsServer::start(17506, 17606, 17706).expect("Failed to start server");
    ...
    assert!(
        conn.is_ok(),
        "Server should be responsive with all extensions"
    );
}
...
fn test_e2e_extension_init_robustness() {
    ...
    match server {
        Ok(s) => {
            thread::sleep(Duration::from_secs(2));
            let conn =
                TcpStream::connect_timeout(&s.address().parse().unwrap(), Duration::from_secs(5));
...
fn test_e2e_namespace_crypto_init() {
    let server = ExtensionsServer::start(17508, 17608, 17708).expect("Failed to start server");
    ...
    assert!(
        conn.is_ok(),
        "Server with namespace crypto should be running"
    );
}
...
fn test_e2e_wasm_registry_init() {
    let server = ExtensionsServer::start(17509, 17609, 17709).expect("Failed to start server");
...
fn test_e2e_mesh_mdns_active() {
    let server = ExtensionsServer::start(17510, 17610, 17710).expect("Failed to start server");
...
fn test_e2e_settrans_virtual_fs() {
    let server = ExtensionsServer::start(17511, 17611, 17711).expect("Failed to start server");
...
fn test_e2e_compute_namespace_ready() {
    let server = ExtensionsServer::start(17512, 17612, 17712).expect("Failed to start server");
    ...
    assert!(
        conn.is_ok(),
        "Server with compute namespace should be running"
    );
}
...
fn test_e2e_concurrent_extension_access() {
    let server = ExtensionsServer::start(17513, 17613, 17713).expect("Failed to start server");
    ...
    assert!(
        success_count >= 2,
        "Should handle concurrent extension access"
    );
}
...
fn test_e2e_auto_mount_daemon_init() {
    let server = ExtensionsServer::start(17514, 17614, 17714).expect("Failed to start server");

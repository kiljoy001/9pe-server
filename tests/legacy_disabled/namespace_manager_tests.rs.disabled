...
use hex;
use ninep_server::{
    namespace_manager::{NamespaceClaim, NamespaceManager, NamespaceMetadata},
...
async fn test_namespace_participant_management() -> Result<()> {
    ...
    // Remove participant
    manager
        .remove_participant("/srv/test/participant", &participant_pubkey, &owner_keypair)
        .await?;
...
async fn test_namespace_access_request_workflow() -> Result<()> {
    ...
    // Approve request
    manager
        .approve_access_request("/srv/test/access", &requester_pubkey, &owner_keypair)
        .await?;
...
async fn test_namespace_liveness_tracking() -> Result<()> {
    ...
    // Add participant
    manager
        .add_participant("/srv/test/liveness", &participant_pubkey, &owner_keypair)
        .await?;

    // Update liveness
    manager
        .update_liveness("/srv/test/liveness", &participant_pubkey)
        .await?;
...
async fn test_namespace_access_request_rejection() -> Result<()> {
    ...
    // Reject request
    manager
        .reject_access_request("/srv/test/reject", &requester_pubkey, &owner_keypair)
        .await?;
...
async fn test_namespace_m_of_n_requirements() -> Result<()> {
    ...
    manager
        .add_participant("/srv/test/mofn", &participant1_pubkey, &owner_keypair)
        .await?;

    manager
        .add_participant("/srv/test/mofn", &participant2_pubkey, &owner_keypair)
        .await?;
    ...
    assert!(updated_claim
        .metadata
        .participants
        .contains(&participant1_pubkey));
    assert!(updated_claim
        .metadata
        .participants
        .contains(&participant2_pubkey));

    // Test that participants can update liveness
    manager
        .update_liveness("/srv/test/mofn", &participant1_pubkey)
        .await?;
    manager
        .update_liveness("/srv/test/mofn", &participant2_pubkey)
        .await?;
...
async fn test_public_namespace_discovery() -> Result<()> {
    ...
    assert_eq!(
        public_namespaces[0].metadata.participant_requirements,
        Some((1, 0))
    );

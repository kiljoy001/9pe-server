...
async fn test_gpu_synthetic_files() -> Result<()> {
    ...
    let gpu_runtimes: Vec<Arc<GpuRuntime>> = gpu_infos
        .iter()
        .map(|gpu_info| Arc::new(GpuRuntime::new(gpu_info.total_vram_bytes)))
        .collect();
    ...
    let info_path = std::path::PathBuf::from("/srv/compute/gpu0/info");
    ...
    let vram_free_path = std::path::PathBuf::from("/srv/compute/gpu0/vram_free");
    assert!(synth_fs.exists(&vram_free_path).await);

    let vram_status_path = std::path::PathBuf::from("/srv/compute/gpu0/vram_status");
    assert!(synth_fs.exists(&vram_status_path).await);

    // Test that compute control files exist
    let submit_path = std::path::PathBuf::from("/srv/compute/submit");
    assert!(synth_fs.exists(&submit_path).await);

    let jobs_path = std::path::PathBuf::from("/srv/compute/jobs");
    assert!(synth_fs.exists(&jobs_path).await);

    let devices_path = std::path::PathBuf::from("/srv/compute/devices");
    assert!(synth_fs.exists(&devices_path).await);

    let status_path = std::path::PathBuf::from("/srv/compute/status");

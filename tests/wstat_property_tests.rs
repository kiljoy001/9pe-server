...
async fn prop_wstat_permission_changes() {
    let strategy = (
        filename_strategy(),
        file_permissions_strategy(),
        file_permissions_strategy(),
    );
    let mut runner = proptest::test_runner::TestRunner::default();

    runner
        .run(&strategy, |(filename, initial_perm, new_perm)| {
            futures::executor::block_on(async {
                ...
                std::fs::set_permissions(&file_path, std::fs::Permissions::from_mode(initial_perm))
                    .unwrap();
...
async fn prop_wstat_file_truncation() {
    ...
    runner
        .run(&strategy, |(filename, initial_size, new_size)| {
            futures::executor::block_on(async {
                // Setup
                let temp_dir = TempDir::new().unwrap();
                let dag = Arc::new(BoundedGhostdag::new("test_consensus".to_string()));
                ...
                if new_size <= initial_size {
                    ...
                    prop_assert_eq!(
                        &final_content[..initial_size as usize],
                        &initial_content,
...
async fn prop_wstat_file_rename() {
    ...
    runner
        .run(&strategy, |(old_name, new_name)| {
            futures::executor::block_on(async {
                ...
                prop_assert!(
                    blocks_added >= 1,
                    "Rename operation should log to consensus DAG"
                );
        ...
        .run(&strategy, |invalid_data| {
            futures::executor::block_on(async {
                ...
                if let Ok(msg) = result {
                    ...
                    prop_assert!(
                        file_path.exists(),
                        "File should still exist after failed wstat"
                    );
                    let content = std::fs::read(&file_path).unwrap();
                    prop_assert_eq!(
                        content,
...
async fn prop_wstat_concurrent_operations_consensus_integrity() {
    let strategy = prop::collection::vec((filename_strategy(), file_permissions_strategy()), 2..5);
    let mut runner = proptest::test_runner::TestRunner::default();
                ...
                for (i, (filename, new_perm)) in operations.iter().enumerate() {
                    ...
                    let handle = tokio::spawn(async move {
                        ...
                        let _ = handler
                            ...
                            .await;
                        let _ = handler
                            .handle_walk(fid, fid + 100, vec![filename.clone()])
                            .await;

...
pub struct User {
    pub uid: u32,
    pub username: String,
    pub password_hash: String,  // Secure password hash (argon2/bcrypt)
...
impl AuthService {
        ...
        match method {
            ...
            AuthMethod::Password(password) => {
                ...
                for (_, user) in users.iter() {
                    // Use secure password verification (no username comparison!)
    ...
    /// Add user with input validation
    pub async fn add_user(&self, mut user: User) -> Result<()> {
        // Validate username
        user.username = validation::validate_username(&user.username)
            .context("Invalid username")?;

        // Validate groups
        for group in &user.groups {
            validation::validate_username(group)
    ...
    pub async fn add_acl(&self, path: String, entry: AclEntry) -> Result<()> {
        // For ACL paths, we allow absolute paths during server setup
        // but still prevent null bytes and check length
        if path.contains('\0') {
            bail!("ACL path contains null byte");
        }

        if path.len() > 4096 {
            bail!("ACL path too long");
        }

        let validated_path = PathBuf::from(&path);

        // Validate username in ACL entry
        validation::validate_username(&entry.principal)
            .context("Invalid principal in ACL entry")?;

        self.acls.write().await
            .entry(validated_path.to_string_lossy().to_string())
    ...
    /// Verify password against Argon2 hash (secure comparison)
    fn verify_password(&self, password: &str, hash: &str) -> bool {
        if hash.is_empty() {
            return false;
        }

        // Handle legacy hash format fallback for transitional period
        if hash.starts_with("production_") {
            // Legacy format - gradually phase this out
            warn!("Legacy password hash detected, consider updating user password");
            return self.verify_legacy_password(password, hash);
        }
        ...
        match PasswordHash::new(hash) {
            ...
            Err(e) => {
                warn!("Invalid password hash format: {}", e);
                false
            }
        }
    }
    ...
    fn verify_legacy_password(&self, password: &str, hash: &str) -> bool {
        use std::collections::hash_map::DefaultHasher;
        use std::hash::{Hash, Hasher};

        let mut hasher = DefaultHasher::new();
        password.hash(&mut hasher);
        let computed_hash = "production_".to_string() + &hasher.finish().to_string();

        // Use constant-time comparison to prevent timing attacks
        let expected_bytes = hash.as_bytes();
        let provided_bytes = computed_hash.as_bytes();
...
mod tests {
    ...
    async fn test_capability_issuance() {
        ...
        let cap = auth.issue_capability(
            ...
            Permissions::READ,
            3600,
        ).await.expect("Failed to issue test capability");

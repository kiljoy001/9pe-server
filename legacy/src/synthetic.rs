...
// Temporarily disable namespace transformers to avoid circular dependency
// TODO: Re-enable after fixing the module structure
// use crate::namespace_transformers::{NamespaceMountGenerator, ServiceRegistryGenerator, NamespaceControlGenerator};
#[cfg(test)]
...
impl SyntheticGenerator for GaussianSplatChartGenerator {
    async fn generate(&self, offset: u64, count: u32) -> Result<Vec<u8>> {
        ...
        let _data = match self.metric_type {
        ...
        let _chart_type = match self.metric_type {
...
pub struct SyntheticFileSystem {
    ...
    // TODO: Re-enable namespace transformers after fixing module dependencies
    // namespace_mount: Arc<NamespaceMountGenerator>,
    // service_registry: Arc<ServiceRegistryGenerator>,
    // namespace_control: Arc<NamespaceControlGenerator>,
}

impl SyntheticFileSystem {
    pub fn new() -> Self {
        Self::with_consensus(None)
    }

    /// Create SyntheticFileSystem with namespace consensus integration
    #[allow(dead_code)]
    pub fn with_consensus(_consensus: Option<()>) -> Self {
        ...
        // TODO: Re-enable namespace transformers after fixing module dependencies
        // Create namespace transformers
        // let namespace_mount = Arc::new(NamespaceMountGenerator::new(consensus.clone()));
        // let service_registry = Arc::new(ServiceRegistryGenerator::new(consensus.clone()));
        // let namespace_control = Arc::new(NamespaceControlGenerator::new(
        //     namespace_mount.clone(),
        ...
        // Register legacy service discovery (for backward compatibility)
        generators.insert("/srv/legacy".to_string(), service_discovery.clone() as Arc<dyn SyntheticGenerator>);

        // TODO: Re-enable namespace transformers after fixing module dependencies
        // Register new namespace transformers - THE CORE PLAN 9 FEATURES!
        // generators.insert("/n".to_string(), namespace_mount.clone() as Arc<dyn SyntheticGenerator>);
        // generators.insert("/n/ctl".to_string(), namespace_control.clone() as Arc<dyn SyntheticGenerator>);
        // generators.insert("/srv".to_string(), service_registry.clone() as Arc<dyn SyntheticGenerator>);
        // generators.insert("/srv/ctl".to_string(), namespace_control.clone() as Arc<dyn SyntheticGenerator>);
    ...
    pub async fn register_service(&self, name: String, info: ServiceInfo) {
        ...
        let _service_file = format!("/srv/{}", name);
    ...
    // TODO: Re-enable namespace transformers after fixing module dependencies
    // /// Get the namespace mount generator
    // pub fn namespace_mount(&self) -> Arc<NamespaceMountGenerator> {
    //     self.namespace_mount.clone()
    // }

    // /// Get the service registry generator
    // pub fn service_registry(&self) -> Arc<ServiceRegistryGenerator> {
    //     self.service_registry.clone()
    // }

    // /// Get the namespace control generator
    // pub fn namespace_control(&self) -> Arc<NamespaceControlGenerator> {
    //     self.namespace_control.clone()
    // }
}

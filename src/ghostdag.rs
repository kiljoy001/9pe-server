...
impl GhostDAG {
    ...
    /// Main GHOSTDAG algorithm - compute blue set
    pub async fn compute_blue_set(&self, tip: &BlockHash) -> HashSet<BlockHash> {
        ...
        match dag.get(tip) {
            ...
            Some(node) => {
                ...
                for parent in &node.block.parent_hashes {
                    // Recursively compute blue set for parent
                    let parent_blue = Box::pin(self.compute_blue_set(parent)).await;
                ...
                // Get best parent's blue set
                let mut blue_set = Box::pin(self.compute_blue_set(&best_parent)).await;

                // Add current block
                blue_set.insert(*tip);

                // Try to add blocks from anticone if they maintain k-cluster property
                let anticone = self.compute_anticone(tip).await;
                for block in anticone {
                    let mut test_set = blue_set.clone();
                    test_set.insert(block);

                    if self.is_k_cluster(&test_set).await {
    ...
    pub async fn add_block(&self, block: Block) -> Result<()> {
        ...
        // Skip blue set recomputation for now to avoid recursion issue
        // TODO: Fix compute_blue_set to avoid infinite recursion

...
use crate::mesh::{get_mesh_network, MeshMessage};
...
impl MeshClient {
    /// Create a new mesh client and start discovery
    ...
    pub async fn list_peers(&self) -> Vec<DiscoveredPeer> {
        // Get peers from the global mesh network
        if let Some(mesh) = get_mesh_network().await {
            mesh.get_discovered_peers().await
        } else {
            // No mesh network running, return empty
            Vec::new()
        }
    }
